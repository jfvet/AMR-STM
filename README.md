# Statistical analyses of antibiotic-resistant bacteria carriage in urban wild small mammals

## Study context

This repository contains the R code used for statistical analyses in an M2 internship study investigating the carriage of antibiotic-resistant bacteria in wild small terrestrial mammals captured in the Lyon metropolitan area (France).
...
3. **Descriptive statistics** = sample sizes by species, site and season; crude prevalence with 95% confidence intervals
...
6. **Individual-level GLMM  = generalized linear mixed models with binomial family and site as random intercept (`lme4`); model selection by AICc using `dredge()` (`MuMIn`); these models are included as a methodological appendix documenting convergence failures and identifiability issues
